...
export const formatDate = (d) => { 
    try { 
        return format(new Date(d), "dd MMM, HH:mm", {locale: ptBR}); 
    } catch { 
        return '-'; 
    } 
};
...
export const StatusBadge = ({ status, inverse }) => {
    ...
    else if (s.includes('negado') || s.includes('cancelado')) color = inverse ? "bg-white/20 text-white border-white/40" : "bg-red-500/10 text-red-700 border-red-500/20";
...
export const getStatusColorBg = (status) => {
    ...
    if(s.includes('pendente')) return "bg-amber-50 dark:bg-amber-950/30";
    return "bg-muted/50";
}

export const InsumoImage = ({ src, alt, sku }) => {
    const [err, setErr] = useState(false);
    if (!src || err) return <div className="w-full h-full bg-gradient-to-br from-muted/20 to-muted/60 flex items-center justify-center"><Box className="w-12 h-12 text-muted-foreground/20" /></div>;
    return <div className="w-full h-full relative bg-white"><img src={src} alt={alt} className="w-full h-full object-contain p-2 mix-blend-multiply transition-transform hover:scale-105" onError={()=>setErr(true)} /></div>;
};
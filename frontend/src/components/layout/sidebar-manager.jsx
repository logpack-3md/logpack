...
export default function SidebarManager({ isOpen, onToggle }) {
  ...
  // Helper: fecha o menu se estiver no mobile (onToggle existe)
  ...
  const getInitials = (name) => name?.split(" ").map((n) => n[0]).join("").toUpperCase().slice(0, 2) || "GP";
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between h-16 px-6 border-b border-border shrink-0">
        <Link href="/dashboard/manager" onClick={handleLinkClick} className="flex items-center gap-3 group outline-none">
        ...
        {/* Botão Fechar (Só aparece se onToggle for passado - Mobile) */}
        {onToggle && (
            <button
            onClick={onToggle}
      ...
      <div className="p-4 border-t border-border bg-muted/30">
        ...
        <div className="flex items-center gap-3 p-2 rounded-lg bg-background border border-border shadow-sm">
          ...
          <div className="flex flex-col overflow-hidden">
            <span className="text-sm font-semibold text-foreground truncate" title={user.name}>{user.name}</span>
            <span className="text-[10px] text-muted-foreground truncate uppercase tracking-wider">Gerente de Produção</span>
          </div>
        </div>
        <div className="mt-2 text-[10px] text-center text-muted-foreground/60">© 2025 LogPack Inc.</div>
      </div>
    </aside>
  );
}
...
async function apiFetchServer(path, options = {}) {
  const { skipAuth = false, ...fetchOptions
    //  cache = "force-cache" AQUI CONFICURA O OPTIONS DAS REQ SENDO FORÇADO OU NÃO
    // next: {revalidade: 3600} NESTA LINHA VOCE CONSEGUE PERSONALIZAR O TEMPO DELE 
  } = options; 

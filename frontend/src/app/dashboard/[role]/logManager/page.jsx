...
import ManagerLogTable from "@/components/Logs/ManagerLogTable";
import { useManagerLogs } from "@/hooks/useManagerLogs";

// Mantemos estrutura manual do mobile para garantir performance
export default function LogManagerPage() {
    const { logs, loading, pagination, fetchLogs } = useManagerLogs();
    ...
    // Handler específico da Tab para resetar paginação
    const handleTabChange = (val) => {
        setCurrentTab(val);
        // Reseta o filtro de ação ao trocar de aba para não confundir
        // setActionFilter('todos'); 
        // O fetchLogs no useEffect já cuida da busca
    };
    ...
    return (
        <div className="flex min-h-screen bg-muted/40 font-sans text-foreground overflow-hidden">
            ...
            {/* SIDEBAR COMPONENTIZADA (gerencia seu próprio Sheet/Overlay no mobile) */}
            ...
            <div className="flex flex-1 flex-col lg:ml-64 transition-all duration-300 w-full min-w-0">
                ...
                <main className="flex flex-1 flex-col p-4 md:p-6 lg:p-8 gap-4 overflow-hidden h-[calc(100vh-4rem)]">
                    ...
                    {/* TOPO DA PAGINA (Títulos + Tabs + Filtro) */}
                    <div className="flex flex-col gap-4 shrink-0">
                        
                        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
                            <div className="space-y-1">
                                <h1 className="text-xl sm:text-2xl font-bold tracking-tight">Histórico de Operações</h1>
                                <p className="text-sm text-muted-foreground">
                                    Monitore as atividades dos processos.
                                </p>
                            </div>
